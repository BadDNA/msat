...
def interface():
    ...
    p.add_option('--min-length', '-m', dest='min_length', default = 6, help=\
    'The minimum length required for repeats to be considered', metavar=\
    'MIN_LENGTH')
    
    p.add_option('--output-file', '-o', dest='output_file', default= None, \
    help='The name of the output file for results [default=None]', metavar=\
    'FILE')
    
    p.add_option('--scan-type', '-s', default='all', help="The type of scan \
to run.  Valid options are:  (1)'all' to search for everything '1' | \
'2' | '3 | '4' | '5' | '6' to search for only mononuc, dinuc, trinuc, \
etc. '2+' | '3+' | '4+' | '5+' | '6+' to search for \
>= dinuc, >= trinuc, etc. [default=all]")
    
    p.add_option('--perfect-repeats', '-p', dest='perfect', action=\
'store_true', default=False, help="Locate only perfect repeats.  The \
default of false only means that when you have a repeat like \
'ACACACACACACNNAC', where ambiguous bases are equal to the exact length \
of the repeat motif (in this case 2), ambiguous bases will be counted as \
part of the repeat.  This is true IFF the ambiguous bases are followed \
...
def genMotifCollection(options):
    ...
    if options.scan_type == 'all':
        for m in possible_motifs:
            motif_collection += (createMotifInstances(m, options.min_length, \
            options.perfect),)
        ...
        scan = int(options.scan_type[0]) - 1
        motif_collection += (createMotifInstances(possible_motifs[scan], \
        options.min_length, options.perfect),)
